...
TextInputDialog::TextInputDialog(const std::string& title, QWidget* parent) :
    QDialog(parent), m_layout(nullptr), m_edit(nullptr), m_ok(nullptr), m_cancel(nullptr), m_title(nullptr)
{
  ...
  connect(m_edit, &QLineEdit::textChanged, this, [this]() { m_result = m_edit->text().toStdString(); });
