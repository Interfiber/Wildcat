...
ImportWindow::ImportWindow()
{
  ...
  for (const auto& pair : WildcatArchiveImporter::get()->getArchivePairs())
  {
    ...
    connect(button, &QRadioButton::clicked, this, [pair, this](bool checked) { m_importButton->setDisabled(false); });
...
void
ImportWindow::openFromPaste(ChannelsWidget* channelsWidget)
{
  ...
  for (int i = 0; i < WildcatArchiveImporter::get()->getArchivePairs().size(); i++)
  {
    ...
    catch (std::exception& e)
    {
      ...
      QMessageBox::warning(this, "Wildcat Importer",
                           ("Error during archive validation: " + std::string(e.what())).data());
    }
  }
