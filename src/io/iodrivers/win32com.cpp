...
std::string WildcatUtil_GetLastError() {
...
std::vector<std::string> WildcatWin32ComDriver::getConnectedDevices()
{
...
void WildcatWin32ComDriver::releaseDevice()
{
...
bool WildcatWin32ComDriver::isConnected()
{
    return m_device != nullptr;
}
 
WildcatIODriver::IOResult WildcatWin32ComDriver::readFromDevice()
{
    std::string buffer;
...
WildcatIODriver::IOResult WildcatWin32ComDriver::writeToDevice(const std::string &buffer)
{
...
WildcatIODriver::IOResult WildcatWin32ComDriver::connectToDevice(const std::string &name)
{
    printf("Win32Com Driver: Connecting to device at: %s\n", name.c_str());
 
    m_device = CreateFileA(("\\\\.\\" + name).c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, 0, nullptr);
 
    if (m_device == INVALID_HANDLE_VALUE)
    {
        return WildcatIODriver::IOResult("Failed to open " + name + " for GENERIC_READ and GENERIC_WRITE: " + WildcatUtil_GetLastError(), true);
    }
    ...
    {
        return WildcatIODriver::IOResult("Failed to call SetCommTimeouts for COM device: " + WildcatUtil_GetLastError(), true);
    }
